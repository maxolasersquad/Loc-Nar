...
_log_msg "Running k9s switch script for version ${version} at location: ${location_path}"

if [ -f "${location_path}/k9s.exe" ]; then
  executable_abs_path="${location_path}/k9s.exe"
else
  executable_abs_path="${location_path}/k9s"
fi
...
_log_msg "k9s switch script finished successfully."
