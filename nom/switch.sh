...
_log_msg "Running nom switch script for version ${version} at location: ${location_path}"

if [ -f "${location_path}/nom.exe" ]; then
  executable_abs_path="${location_path}/nom.exe"
else
  executable_abs_path="${location_path}/nom"
fi
...
_log_msg "nom switch script finished successfully."
