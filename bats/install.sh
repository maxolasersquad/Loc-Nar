...
location_path=""
verbose=0
github_api_url="https://api.github.com/repos/bats-core/bats-core/releases"
...
if [ "${source_install}" -eq 1 ]; then
  _log_msg "Installing from source (requested via --source)." >&2
fi
...
if ! command -v bash >/dev/null 2>&1; then
  _error_msg "'bash' command not found. bats requires bash to run."
  return 6
fi
...
tmp_dir=$(mktemp -d /tmp/trk-bats-install-XXXXXX) || {
...
get_specific_release_url() {
  ...
  if [ -z "${download_url}" ] || [ "${download_url}" = "null" ]; then
    _error_msg "Could not find a suitable download URL for bats version ${version}."
    _error_msg "Check if version exists and has the expected tarball at GitHub."
    return 2
  fi
...
download_and_install() {
  download_url="${1}"
  download_file="${tmp_dir}/bats-core-${version}.tar.gz"
  install_cmd_status=1

  _log_msg "Downloading bats from ${download_url}" >&2
  if command -v curl >/dev/null 2>&1; then
  ...
  _log_msg "Found source directory: ${extracted_dir}" >&2

  _log_msg "Running bats installer..." >&2
  install_ok=0
  if [ "${verbose}" -eq 1 ]; then
    (cd "${extracted_dir}" && ./install.sh "${location_path}") && install_ok=1
  else
    (cd "${extracted_dir}" && ./install.sh "${location_path}") >/dev/null 2>&1 && install_ok=1
  fi

  if [ "${install_ok}" -eq 1 ]; then
    install_cmd_status=0
  else
    _error_msg "bats install.sh failed."
    return 5
  fi

  if [ "${install_cmd_status}" -ne 0 ]; then
    _error_msg "Failed to install bats to ${location_path} (Exit code: ${install_cmd_status})."
    return 1
  fi

  _log_msg "bats installed successfully to ${location_path}" >&2
