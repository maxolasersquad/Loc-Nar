...
_log_msg "Running shellcheck switch script for version ${version} at location: ${location_path}"

if [ -f "${location_path}/shellcheck.exe" ]; then
  executable_abs_path="${location_path}/shellcheck.exe"
else
  executable_abs_path="${location_path}/shellcheck"
fi
...
_log_msg "shellcheck switch script finished successfully."
