...
location_path=""
verbose=0
github_api_url="https://api.github.com/repos/koalaman/shellcheck/releases"
...
if [ "${source_install}" -eq 1 ]; then
  if ! command -v cabal >/dev/null 2>&1; then
    _error_msg "'cabal' command not found. Required for source install."
    return 6
  fi
  if ! command -v ghc >/dev/null 2>&1; then
    _error_msg "'ghc' command not found. Required for source install."
    return 6
  fi
else
  if [ "${os}" = "windows" ]; then
...
tmp_dir=$(mktemp -d /tmp/trk-shellcheck-install-XXXXXX) || {
...
get_specific_release_url() {
  ...
  if [ "${source_install}" -eq 1 ]; then
    ...
    if [ "${os}" = "windows" ]; then
      ext=".zip"
      query=".assets[] | select(.name | endswith(\"${ext}\") and (contains(\"darwin\") | not) and (contains(\"linux\") | not)) | .browser_download_url"
    else
  ...
  if [ -z "${download_url}" ] || [ "${download_url}" = "null" ]; then
    _error_msg "Could not find a suitable download URL for shellcheck version ${version} (os=${os}, arch=${arch}, source=${source_install})."
    return 2
  fi
...
download_and_install() {
  ...
  _log_msg "Downloading shellcheck from ${download_url}" >&2
  if command -v curl >/dev/null 2>&1; then
  ...
  if [ "${source_install}" -eq 1 ]; then
    _log_msg "Building shellcheck from source…" >&2
    SHELLCHECK_BUILD_DIR=$(find "${tmp_dir}" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | head -n 1)

    if [ -z "${SHELLCHECK_BUILD_DIR}" ] || [ ! -d "${SHELLCHECK_BUILD_DIR}" ]; then
      _error_msg "Could not find extracted source directory in ${tmp_dir}"
      return 4
    fi
    _log_msg "Found source directory: ${SHELLCHECK_BUILD_DIR}" >&2

    _log_msg "Running cabal install…" >&2
    build_ok=0
    if [ "${verbose}" -eq 1 ]; then
      (cd "${SHELLCHECK_BUILD_DIR}" && cabal update && cabal install --installdir="${location_path}" --install-method=copy --overwrite-policy=always) && build_ok=1
    else
      (cd "${SHELLCHECK_BUILD_DIR}" && cabal update && cabal install --installdir="${location_path}" --install-method=copy --overwrite-policy=always) >/dev/null 2>&1 && build_ok=1
    fi
    ...
    if [ "${build_ok}" -eq 1 ]; then
      ...
      _error_msg "cabal build/install failed."
      return 5
    fi
  else
    _log_msg "Installing pre-compiled binary…" >&2
    shellcheck_binary=$(find "${tmp_dir}" \( -name shellcheck -o -name shellcheck.exe \) -type f 2>/dev/null | head -n 1)
    if [ -z "${shellcheck_binary}" ]; then
      _error_msg "Could not find 'shellcheck' executable in extracted archive."
      return 4
    fi
    _log_msg "Found binary: ${shellcheck_binary}" >&2

    install -m 755 "${shellcheck_binary}" "${location_path}/"
    install_cmd_status=$?
  fi
  ...
  if [ -f "${location_path}/shellcheck.exe" ]; then
    printf '%s\n' "shellcheck.exe"
  else
    printf '%s\n' "shellcheck"
  fi
