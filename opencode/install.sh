...
if [ "${source_install}" -eq 1 ] && ! command -v cargo >/dev/null 2>&1; then
  _error_msg "'cargo' command not found."
  exit 6
fi
...
download_and_install() {
  ...
  if [ "${source_install}" -eq 1 ]; then
    ...
    build_ok=0
    if [ "${verbose}" -eq 1 ]; then
      (cd "${OPENCODE_BUILD_DIR}" && go build -ldflags "-s -w -X main.date=$(date -u +'%Y-%m-%dT%H:%M:%SZ') -X main.version=${version} -X main.buildSource=tarball" -o opencode) && build_ok=1
    else
      (cd "${OPENCODE_BUILD_DIR}" && go build -ldflags "-s -w -X main.date=$(date -u +'%Y-%m-%dT%H:%M:%SZ') -X main.version=${version} -X main.buildSource=tarball" -o opencode) >/dev/null 2>&1 && build_ok=1
    fi
